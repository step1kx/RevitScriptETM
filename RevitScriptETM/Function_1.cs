...
namespace RevitScriptETM
{
    ...
    public class Function_1 : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            // Устанавливаем обработчик AssemblyResolve
        ...
        private Assembly ResolveAssembly(object sender, ResolveEventArgs args)
        {
            try
            {
                // Получаем директорию, где находится плагин
                string pluginDirectory = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                string assemblyFileName = new AssemblyName(args.Name).Name + ".dll";
                string assemblyPath = Path.Combine(pluginDirectory, assemblyFileName);
            ...
            {
                MessageBox.Show($"Ошибка загрузки сборки: {args.Name}\n{ex.Message}");
            }

            return null; // Возвращаем null, если сборка не найдена
        }
    }
}
